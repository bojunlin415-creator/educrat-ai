...
const liveDescribe =
  process.env.LEARNER_REPORTING_LIVE_VERIFY === "1" ? describe : describe.skip;
...
liveDescribe(
  "LE-001 Phase 5C Development Reporting population evidence",
  () => {
    it("verifies aggregate canonical population and compatibility evidence read-only", async () => {
      ...
      try {
        ...
        const metrics = compareReportingPopulationAuthorities({
          classIds,
          snapshot,
        });
        ...
        console.info(
          "[reporting-population-live]",
          JSON.stringify({ metrics, projectRef }),
        );
        ...
        expect(metrics.metricCompatibilityCount).toBeLessThanOrEqual(
          metrics.canonicalPopulationCount,
        );
        expect(
          metrics.canonicalLearnersWithoutLegacyMetrics,

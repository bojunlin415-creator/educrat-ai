...
const ROOT = path.join(process.cwd(), "lib/recycle-bin");
...
describe("recycle bin architecture", () => {
  ...
  it("keeps layer dependencies pointed inward", () => {
    const allowedByLayer: Record<string, readonly string[]> = {
      application: [
        "@/lib/recycle-bin/application",
        "@/lib/recycle-bin/domain",
        "@/lib/recycle-bin/interfaces",
        "@/lib/recycle-bin/shared",
      ],
      domain: ["@/lib/recycle-bin/domain", "@/lib/recycle-bin/shared"],
      interfaces: ["@/lib/recycle-bin/domain", "@/lib/recycle-bin/interfaces"],
      shared: ["@/lib/recycle-bin/shared"],
    };
    ...
    for (const file of productionFiles()) {
      ...
      const imports = importsOf(file).filter((specifier) =>
        specifier.startsWith("@/lib/recycle-bin"),
      );

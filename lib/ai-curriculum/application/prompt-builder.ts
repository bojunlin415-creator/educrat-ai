...
export function buildCurriculumPrompt(
  ...
  return createCurriculumPrompt({
    messages: [
      ...
      {
        content: [
          `學習階段：${input.learningStage}`,
          `年級：${input.grade} 年級`,
          `科目：${input.subject}`,
          `學習主題：${input.curriculumTopic}`,
          `單元：${input.unit}`,
          `教材用途：${input.purpose}`,
          ...
          "能力指標：",
          input.competencyIndicators.map((item) => `- ${item}`).join("\n"),
          "教學目標：",
          input.learningObjectives.map((item) => `- ${item}`).join("\n"),
          "知識點：",
          knowledgePointLines,

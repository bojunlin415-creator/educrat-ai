...
- Date: 2026-08-10
- Change class: Documentation-only architecture review
- Runtime, database, migration, and Production status: Not changed or validated by this task
...
| Generation-Only Subjects | Existing original generation, structured validation, teacher editing, publish lifecycle, PDF export, and capability-neutral service boundaries are reusable. Word export and centralized subject gating are absent.                                                                                                             | **Closest to current implementation; requires capability enforcement and subject validation.** |
...
| AI curriculum/generation modules                                     | Extend                 | Provider interface, structured output, copyright validation, prompt versioning, and teacher-edit flow are reusable. `grade` is numeric/elementary-oriented and generation lacks subject capability and pathway context.      |
| Curriculum publish/export                                            | Extend                 | Stored-version-only export, lifecycle, PDF renderer, and human review are reusable. Add capability/quality gates and Word renderer later.                                                                                    |
...
| Subject capability runtime                                           | Replace later          | No runtime registry/evaluator exists. Current AI selection exposes all seeded subjects and Assignment/Analytics services do not enforce subject capability. A centralized fail-closed boundary must precede subject rollout. |
...
1. No centralized, versioned subject capability registry or server enforcement.
...
12. Generation-only subject restrictions are documentation, not a central server-side decision.
...
21. Tests do not yet cover a capability matrix, CEFR/pathway/course mapping, Math correctness, generation-only negative gates, durable review-before-publish, or an AI → publish → assignment end-to-end path.
...
1. **S8V-001 — Classes & Students Development Verification**: review/apply only the pending Sprint 8 migration to the approved Development project, verify objects/RLS/API session behavior, and record runtime results. No schema redesign.
2. **CAP-001 — Subject Capability Registry Foundation**: implement the framework-neutral vocabulary, immutable versioned profiles, pure capability evaluator, server projection, and matrix/import-boundary tests. No subject engine or database table.
3. **LE-001 — Canonical Learner & Enrollment Convergence Design/Adapter**: define Student↔Person/Account linkage and Class/Course Enrollment authority, inventory all Profile-based consumers, implement read-only parity/adapters first, and propose (not silently execute) a forward-only backfill/cutover package.
...
The migration is currently untracked in the local working tree and documentation marks the Sprint as awaiting Product Verification. Its static contract test inspects SQL text, and the current E2E evidence does not prove authenticated CRUD, RLS, cross-tenant isolation, or Teacher scope against an applied Development schema. This review did not query or change any database and did not apply the migration. Therefore Development application status and runtime behavior are **not verified** and must be treated as a blocking verification gate.

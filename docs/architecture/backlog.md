...
- 狀態：**LE-001 Phase 1–5E — SEALED；Phase 5F — Implementation and Development verification complete, awaiting package seal**。
...
- Authority：Class detail、Teacher Dashboard、RP-001 Teacher Reporting current population、Assignment Class expansion、新Assignment recipient與new canonical Submission ownership已使用`students.id`／`student_class_members.id`；歷史`assignment_students`／Submission、Learning Events、Mastery、Adaptive、Guardian/Parent與歷史metrics仍為legacy compatibility boundary。
...
- Phase 5E：新增獨立`assignment_student_recipients`與Class provenance，不重用或改寫legacy欄位；新recipient以canonical Student原子寫入，只有verified Account link才建立legacy Submission mirror，accountless learner不因缺mirror失敗。歷史recipient保持`LEGACY_ONLY_HISTORICAL`，raw Profile/Account ID不出server boundary。
- Phase 5F：新增canonical Submission ownership與trusted self resolver；new write需verified Account link＋canonical recipient且原子保存，無write fallback。Historical Profile-keyed row不rewrite，無mapping者保留legacy read；mapped row在link失效後fail closed。
- Package seal gate：Phase 5F受控Development fixture已完成canonical save/read-back、accountless、revoked、expired、cross-tenant、non-recipient、spoofing、anonymous與Teacher scope驗證；五個臨時Auth帳號及marker資料已全數清除。Learning Event、Mastery、Adaptive、Guardian/Parent、legacy freeze與destructive cleanup仍未開始。
- Development：Phase 5F migration只套用`educrat-development`；31/31 history同步且post-apply dry-run up to date。新Submission具canonical ownership且不依賴legacy recipient；失效link阻擋未來寫入但保留歷史資料。Production未操作，runtime維持`CANONICAL_PRIMARY_LEGACY_FALLBACK`。

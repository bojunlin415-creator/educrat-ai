...
const route = "/api/access/members/[membershipId]";
...
export async function DELETE(
  ...
  const parsed = await parseAccessControlJson(
    request,
    removeAccessRoleSchema.omit({ membershipId: true }),
  );
  const membershipId = removeAccessRoleSchema.shape.membershipId.safeParse(
    rawMembershipId,
  );
  if (!parsed.success) {
    logAccessApi({
  ...
  try {
    ...
    const removedMembershipId = await removeAccessRole(serviceInput);
    logAccessApi({
      payload: parsed.body,
      response: { membershipId: removedMembershipId, status: 200 },
      route,
      service: { input: serviceInput, output: { removedMembershipId } },
      validation: {
        membershipId: membershipId.data,
        payload: parsed.data,
        success: true,

...
import { getTeacherDashboardClasses } from "@/lib/teacher-dashboard/service";
...
export async function GET(request: Request) {
  ...
  try {
    const classes = await getTeacherDashboardClasses(parsed.data);
    return Response.json(
      teacherDashboardSuccess("教師班級表現已載入。", { classes }),
    );
  } catch (error: unknown) {

...
lib/learner-convergence/ LE-001 learner discrepancy／parity、controlled backfill、shadow comparison、consumer cutover readiness planning 與 canonical Student self-resolution boundary
...
- [LE-001 Phase 3 Controlled Backfill & Enrollment Parity](docs/architecture/le-001-phase-3-controlled-backfill-enrollment-parity.md)：server/operator-only deterministic candidate、dry-run、idempotent link/enrollment backfill contracts 與 Development 零寫入 review
...
- [LE-001 Phase 5C Reporting Learner Population](docs/architecture/le-001-phase-5c-reporting-learner-population.md)：RP-001 Teacher Reporting learner/class population改用canonical roster與canonical Student key；既有metrics與歷史Profile key仍為compatibility authority
- [LE-001 Phase 5D Assignment Class Expansion](docs/architecture/le-001-phase-5d-assignment-class-expansion.md)：Assignment class target learner population改用canonical roster；`assignment_students`與Submission仍維持legacy compatibility authority
- [LE-001 Phase 5E Assignment Recipient Canonicalization](docs/architecture/le-001-phase-5e-assignment-recipient-canonicalization.md)：新Assignment recipient以`students.id`持久化，保留歷史Profile-keyed recipient與Submission compatibility，不暴露raw legacy identity
- [LE-001 Phase 5F Submission Self-Resolution](docs/architecture/le-001-phase-5f-submission-self-resolution.md)：Submission self identity改由verified Account link解析canonical Student，新增明確canonical ownership與historical compatibility boundary
...
- **LE-001 Phase 3 — DEVELOPMENT VERIFIED AND PACKAGE SEALED**；已建立 authoritative-evidence candidate classifier、immutable dry-run、idempotent/conflict-safe operator executor與 enrollment parity/backfill contract。Development 目前 8 位 Student 均無 authoritative Account evidence，2 筆 canonical-only membership 均 identity unresolved，因此沒有實際 backfill；consumer authority、dual-read/write 與 legacy freeze 均未開始。
- **LE-001 Phase 4 — SEALED**；已建立十個 consumer 共用的 shadow parity model、default-disabled application control、單一 batched snapshot、safe diagnostic 與 non-blocking integration。Development 的 Class／Teacher Dashboard／Reporting 原始 Phase 4 matrix 為 `BLOCKED_IDENTITY`，其餘無資料 consumer 為 `NOT_READY`。
- **LE-001 Phase 5 — Consumer Authority Cutover Planning & Readiness Gates**；新增 `le-001.cutover.v1` readiness evaluator、dependency/package graph、11 個 default-legacy technical controls、Teacher／Student／Guardian／Owner/Admin minimal snapshot contracts 與 5A～5J plan。Class roster 不要求 Account link，但目前仍因 Teacher assigned-class least-privilege boundary 與資料 fixture 不足而 `BLOCKED_SECURITY`；Teacher Dashboard／Reporting 為 `BLOCKED_IDENTITY`，其他 consumer 為 `NOT_READY`。Legacy runtime authority、dual-write、consumer cutover、freeze、destructive cleanup與Production均未變更。
- **LE-001 Phase 5A — Class Roster Canonical Read Cutover**；僅 `GET /api/classes/[id]` roster 使用 `students`／`student_class_members`，選定 `CANONICAL_PRIMARY_LEGACY_FALLBACK`。Owner live read、anonymous／cross-tenant、minimal projection、managed/accountless、fallback／rollback 與 batched-query 驗證通過；Development 無安全 Admin／assigned Teacher fixture，因此未升級 `CANONICAL_ONLY`。無 Migration、dual-write、Production 操作或 Phase 5B。
- **LE-001 Phase 5B — SEALED**；Teacher Dashboard current learner population與per-Class count已使用canonical batched roster，其他Dashboard metrics仍為legacy/compatibility authority。
- **LE-001 Phase 5C — Reporting Learner Population Cutover**；RP-001 Teacher Reporting current learner/class population與internal learner key改用`students`／`student_class_members`，選定`CANONICAL_PRIMARY_LEGACY_FALLBACK`。legacy metrics只接受authoritative mapping，無mapping即維持unavailable，不暴露Profile ID。Development目前active canonical/legacy population均為0；Admin與Teacher live fixtures不可用，因此不升級`CANONICAL_ONLY`。無Migration、dual-write、Production操作或Phase 5D。
- **LE-001 Phase 5D — Assignment Class Expansion Cutover**；Class-target learner expansion改用`student_class_members`＋`students`並選定`CANONICAL_PRIMARY_LEGACY_FALLBACK`。跨班canonical Student會去重；active/left、Class tenant與assigned-Teacher scope fail closed。`assignment_students`仍是legacy Profile/Account recipient authority，無verified compatibility mapping時在任何寫入前回傳typed `recipient_identity_unavailable`，不偽造或誤寫canonical ID。Development current active canonical/legacy candidates為0/0；live mutation fixtures不可用，故不升級`CANONICAL_ONLY`。無Migration、dual-write、Production操作或Phase 5E。
- **LE-001 Phase 5E — Assignment Recipient Canonicalization**；新增canonical recipient、Class provenance與server-only verified legacy compatibility schema，新Assignment以`students.id`原子持久化；managed/accountless Student不需要Profile即可成為recipient。讀寫authority皆明確，canonical write不做legacy fallback，Submission/self-resolution與歷史`assignment_students`仍維持legacy compatibility。Development migration已同步（30/30）、live ACL/RPC/canonical table health通過；因無active Assignment candidate，managed/accountless live mutation為`NOT EXECUTED — FIXTURE UNAVAILABLE`，故維持`CANONICAL_PRIMARY_LEGACY_FALLBACK`且不開始Phase 5F。
- **LE-001 Phase 5F — Implementation and Development verification complete**；新增canonical Submission ownership、verified Account-link self resolver、canonical recipient eligibility、atomic save/submit RPC與revocation-safe historical policy。Development migration同步（31/31）；受控臨時fixture驗證canonical create/read-back、零legacy recipient依賴、accountless／expired／revoked／non-recipient／cross-tenant／spoofing／anonymous拒絕及assigned/unassigned Teacher scope，完成後5個臨時Auth帳號與所有marker資料均已清除。維持`CANONICAL_PRIMARY_LEGACY_FALLBACK`，不升級`CANONICAL_ONLY`，Production未操作。
...
- PB-001：**Implementation Completed — Awaiting Product Review**；已建立正式 `draft`／`in_review`／`published`／`archived` 教材生命週期、server-side submit/review/publish/archive/reopen/new-version API、發布前 validation、版本鎖與 `CURRICULUM_SUBMITTED`／`CURRICULUM_REVIEWED`／`CURRICULUM_PUBLISHED` audit integration。
